    ...
    <>
      ...
      >
        ...
        <div className="p-4 border-t border-[#1C2333]">
           ...
           >
              <div 
                className={`absolute w-1/2 h-8 bg-teal-500 rounded-full transition-all duration-300 ease-[cubic-bezier(0.87,0,0.13,1)] shadow-lg shadow-teal-500/20 ${role === 'admin' ? 'translate-x-[96%]' : 'translate-x-[4%]'}`}
              />
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0D1117]/95 backdrop-blur-md border-t border-[#1C2333] px-2 flex justify-around items-center h-[72px] pb-safe">
          ...
          <button 
            onClick={() => setRole(role === 'admin' ? 'viewer' : 'admin')}
            className={`flex flex-col items-center justify-center w-16 h-full gap-1 transition-colors ${role === 'admin' ? 'text-teal-400 font-bold' : 'text-slate-500 font-medium'}`}
          >
             <User size={20} />
             <span className="text-[10px] tracking-wide">{role.toUpperCase()}</span>
          </button>
      </div>
    </>
  );
